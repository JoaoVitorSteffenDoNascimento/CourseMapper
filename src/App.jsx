...
const getSettingsForm = (user, themeOverride = '') => ({
  name: user?.name || '',
...
const groupBySemester = (subjects) => subjects.reduce((groups, subject) => {
...
const getNextSubjects = (subjects) => subjects.filter((subject) => subject.status === 'available').slice(0, 4);
const getCriticalSubjects = (subjects) => subjects.filter((subject) => subject.isCritical);
const getTrailSummary = (subjects) => Object.entries(subjects.reduce((accumulator, subject) => {
...
function getTrailOrder(subjects, trailLabels = []) {
...
function buildBoardLayout(subjects, trailOrder, semesters) {
...
function getBoardConnectorPath(source, target) {
